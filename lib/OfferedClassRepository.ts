...
export class OfferedClassRepository extends Repository<OfferedClass> {
  async updateClasses(classFN: string) {
    ...
    for (let updater of offered.classes) {
      if (!OfferedClassRepository.isOfferedClassUpdater(updater)) {
        throw new Error(
          `updateClasses found classes entry that is not an OfferedClassUpdater ${util.inspect(
            updater
          )}`
        );
      }
  ...
  async findOneClass(code: string): Promise<OfferedClass> {
    ...
    if (!OfferedClassRepository.isOfferedClass(cls)) {
      throw new Error(
        `OfferedClass id ${util.inspect(code)} did not retrieve a OfferedClass`
      );
    }
    return cls;
  }
  async updateOfferedClass(
    ...
    if (!OfferedClassRepository.isOfferedClassUpdater(offeredClass)) {
      throw new Error(
        `OfferedClass update id ${util.inspect(
          code
        )} did not receive a OfferedClass updater ${util.inspect(offeredClass)}`
      );
    }
  ...
  async deleteOfferedClass(offeredClass: string | OfferedClass) {
    ...
    await this.manager.delete(
      OfferedClass,
      typeof offeredClass === "string" ? offeredClass : offeredClass.code
    );
  }
  async enrollStudentInClass(studentid: any, code: string) {
    ...
    if (!OfferedClassRepository.isOfferedClass(offered)) {
      throw new Error(
        `enrollStudentInClass did not find OfferedClass for ${util.inspect(
          code
        )}`
      );
    }
    ...
    if (!StudentRepository.isStudent(student)) {
      throw new Error(
        `enrollStudentInClass did not find Student for ${util.inspect(
          studentid
        )}`
      );
    }
